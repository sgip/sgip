...
class User(DeclarativeBase):
    ...
    user_fullname = Column(Unicode(40), nullable=False)

    _password = Column('password', Unicode(80), info={'rum': {'field':'Password'}}, nullable=False)

    user_telefono = Column(Unicode(15), nullable=False)

    user_direccion = Column(Unicode(30), nullable=False)

    email_address = Column(Unicode(30), unique=True, nullable=False,
                           info={'rum': {'field':'Email'}})

    #{ Special methods

    def __repr__(self):
        return ('<User: id=%r, name=%r, email=%r' % (
                self.user_id, self.user_name, self.email_address)).encode('utf-8')
